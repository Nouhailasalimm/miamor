...
function createHPBar(unit, hp, isEnemy) {
    ...
    let hpBar = unit.querySelector(".unit-hp");
    ...
    if (!hpBar) {
        ...
        /* -----------------------------------------
           INLINE STYLING
           This means the bars work even without
           additional CSS.
        ----------------------------------------- */
        ...
        hpBar.style.background = "rgba(15, 8, 5, .9)";
        hpBar.style.border = "1px solid rgba(220, 190, 145, .65)";
        hpBar.style.pointerEvents = "none";
        hpBar.style.zIndex = "60";
        hpBar.style.boxSizing = "border-box";

        const fill = hpBar.querySelector(".unit-hp-fill");

        fill.style.display = "block";
        fill.style.width = "100%";
        fill.style.height = "100%";
        fill.style.transition = "width .35s ease";
        fill.style.background = isEnemy
            ? "#8f3028"
            : "#58748a";
...
function updateHPBar(unit, hp) {
    ...
    const hpBar = unit.querySelector(".unit-hp");
...
battlefield.addEventListener("click", function (event) {
    ...
    x = Math.max(
        ...
        Math.min(
            x - unitWidth / 2,
            battlefield.clientWidth - unitWidth - 10
        )
    );
    ...
    y = Math.max(
        ...
        Math.min(
            y - unitHeight / 2,
            battlefield.clientHeight - unitHeight - 10
        )
    );
    ...
    selectedUnit.classList.add("moving");

    setTimeout(function () {

        selectedUnit.classList.remove("moving");
...
function attackEnemy(enemy) {
    ...
    if (selectedUnit.classList.contains("defeated")) {
        return;
    }
    ...
    if (selectedUnit.dataset.attacking === "true") {
        return;
    }

    selectedUnit.dataset.attacking = "true";
    ...
    setTimeout(function () {
        ...
        selectedUnit.dataset.attacking = "false";
...
function findClosestPlayer(enemy) {
    ...
    const livingPlayers = Array.from(playerUnits).filter(
        function (unit) {
...
function enemyAttack(enemy) {
    ...
    const player = target.unit;

    if (target.distance > ENEMY_ATTACK_RANGE) {
        return;
    }
    ...
    setTimeout(function () {
        ...
        let hp = parseInt(
            player.dataset.hp || PLAYER_MAX_HP
        );
...
function defeatPlayer(player) {
    ...
    if (player.classList.contains("defeated")) {
        return;
    }
    ...
    armyCount.textContent =
        Math.max(
            0,
            parseInt(armyCount.textContent || "3") - 1
        );


    if (selectedUnit === player) {
